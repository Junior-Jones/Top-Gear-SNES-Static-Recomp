...
int main(int argc,char**argv){
 TopGearRecomp*c=0;TopGearRecompFrameResult result;uint8_t*rom=0;size_t bytes;char err[256],logpath[1024];unsigned f;int16_t pcm[2048];FILE*log;
 if(argc!=4)return 2;if(!load_file(argv[1],&rom,&bytes)||!topgear_recomp_create(&c,rom,bytes,err,sizeof(err)))return 2;
 c->mod_music_sequence=(uint8_t)atoi(argv[3]);
 snprintf(logpath,sizeof(logpath),"%s/career-route.csv",argv[2]);log=fopen(logpath,"wb");fprintf(log,"frame,page,active,race,engine,sequence,selector,pc\n");
 for(f=0;f<4100;f++){
 uint16_t input=0;
 if(f>=3600)input=TOPGEAR_INPUT_B;
 if(f>=1560&&f<1680)input=TOPGEAR_INPUT_START;
 if((f>=1920&&f<1950)||(f>=2190&&f<2220))input=TOPGEAR_INPUT_A;
 if((f>=2010&&f<2025)||(f>=2070&&f<2085)||(f>=2130&&f<2145))input=TOPGEAR_INPUT_DOWN;
 if(!topgear_recomp_advance_headless(c,input,0,1,&result)){puts(topgear_recomp_last_error(c));return 1;}
 while(topgear_recomp_audio_read(c,pcm,1024)){}
 if(f>=3300){uint8_t eon=0;if(!topgear_recomp_read_smp_dsp_register(c,0x4Du,&eon)||(eon&0xF0u)){fprintf(stderr,"unexpected native effect echo mask %02X\n",eon);return 7;}}
 if(f==2000||f==2170||f==2500||f==4000)screenshot(c,argv[2],f);
 if(f%30==0)fprintf(log,"%u,%u,%u,%u,%u,%u,%u,%02X:%04X\n",f,c->mod_menu_remodel_page,c->mod_menu_remodel_active,c->mod_music_race,c->mod_music_engine,c->mod_music_sequence,c->mod_music_native_selector,c->cpu.pbr,c->cpu.pc);
 }
 printf("race=%u engine=%u sequence=%u selector=%u\n",c->mod_music_race,c->mod_music_engine,c->mod_music_sequence,c->mod_music_native_selector);
 if(!c->mod_music_race||c->mod_music_sequence!=(atoi(argv[3])+1)%11||c->mod_music_engine!=(atoi(argv[3])<4?0:atoi(argv[3])<7?1:2))return 6;
 fclose(log);topgear_recomp_destroy(c);free(rom);return 0;
}
