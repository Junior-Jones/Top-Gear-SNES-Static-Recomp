...
struct TopGearRecomp {
    ...
    /* Explore - Insert audio accessibility-mod state.  Appended so the
       existing static-core member layout remains unchanged for older fields. */
    uint8_t mod_insert_audio_menu_resource_installed;
    uint8_t mod_insert_audio_main_menu_active;
    uint8_t mod_insert_audio_last_menu_selection;
    uint8_t mod_insert_audio_sfx_state;
    uint8_t mod_insert_audio_sfx_command;
    uint8_t mod_insert_audio_sfx_queued;
    uint8_t mod_insert_audio_reserved[2];
    uint64_t mod_insert_audio_resource_install_count;
    uint64_t mod_insert_audio_navigation_change_count;
    uint64_t mod_insert_audio_command_start_count;
    uint64_t mod_insert_audio_command_ack_count;
    uint64_t mod_insert_audio_command_clear_count;
    uint64_t mod_insert_audio_neutral_ack_count;
    /* Part 2: imported SimCity navigation cue state. */
    uint8_t mod_insert_audio_saved_voice7[8];
    uint8_t mod_insert_audio_saved_eon;
    uint8_t mod_insert_audio_simcity_resource_installed;
    uint8_t mod_insert_audio_part2_reserved[6];
    uint64_t mod_insert_audio_cue_keyoff_master_clock;
    uint64_t mod_insert_audio_cue_restore_master_clock;
    uint64_t mod_insert_audio_cue_keyon_count;
    uint64_t mod_insert_audio_cue_keyoff_count;
    uint64_t mod_insert_audio_cue_restore_count;
    /* First-stage core-owned menu remodel state. */
    uint8_t mod_menu_remodel_active;
    uint8_t mod_menu_remodel_page;
    uint8_t mod_menu_remodel_selection;
    uint8_t mod_menu_remodel_dirty;
    uint8_t mod_menu_remodel_return_pending;
    uint8_t mod_menu_remodel_return_page;
    uint8_t mod_menu_remodel_return_selection;
    uint8_t mod_menu_remodel_reserved;
    /* Canonical setup banks: Career/Rally share two players; TT owns P1.
       Each player stores 16 control/gear bytes, 8 name bytes and 2 car bytes.
       Included in the build-owned snapshot image, never in TGTT history. */
    uint8_t mod_setup_initialized;
    uint8_t mod_settings_kph;
    uint8_t mod_audio_mute;
    uint8_t mod_audio_menu_level;
    uint8_t mod_audio_menu_context;
    uint8_t mod_audio_reserved[2];
    uint8_t mod_music_title,mod_music_engine,mod_music_r_held,mod_music_race;
    uint8_t mod_music_mode,mod_music_sequence,mod_music_native_selector;
    uint32_t mod_music_rng;
    uint8_t mod_music_bags[3][11],mod_music_positions[3],mod_music_last[3];
    uint8_t mod_music_state_ready,mod_music_selected_song;
    uint32_t mod_music_random_states[3];
    uint32_t mod_rally_preview_deadline;
    uint16_t mod_rally_preview_frames;
    uint8_t mod_rally_preview_released;
    uint8_t mod_setup_time_trial_active;
    uint8_t mod_setup_import_pending;
    uint8_t mod_profile_pending_mask,mod_profile_pending[2][26];
    uint8_t mod_setup_shared[2][26];
    uint8_t mod_setup_time_trial[26];
    uint16_t mod_setup_shared_players;
    uint16_t mod_setup_shared_courses[2];
    uint64_t mod_menu_remodel_render_count;
    uint64_t mod_menu_remodel_navigation_count;
    uint64_t mod_menu_remodel_accept_count;
    /* Rally mode: one randomly selected canonical course ordinal per country. */
    uint8_t mod_rally_active;
    uint8_t mod_rally_schedule_valid;
    uint8_t mod_rally_race_index;
    uint8_t mod_rally_qualification_pending;
    uint8_t mod_rally_schedule[8];
    uint32_t mod_rally_rng_state;
    uint64_t mod_rally_generation_count;
    /* Time Trial: the static core owns only the live race and one completed-run
       handoff. Historical leaderboard collections and Data/time-trial.dat are
       host-side. No wall-clock timing enters the core. */
    uint8_t mod_time_trial_active;
    uint8_t mod_time_trial_selected_course;
    uint8_t mod_time_trial_country;
    uint8_t mod_time_trial_track;
    uint8_t mod_time_trial_times_course;
    uint8_t mod_time_trial_has_run;
    uint8_t mod_time_trial_lap_count;
    uint8_t mod_time_trial_required_laps;
    uint16_t mod_time_trial_lap_start_tick;
    uint16_t mod_time_trial_last_tick;
    uint16_t mod_time_trial_sector1_tick;
    uint16_t mod_time_trial_sector2_tick;
    uint8_t mod_time_trial_sector_mask;
    uint8_t mod_time_trial_completed_run_valid;
    uint8_t mod_time_trial_completed_run_pending;
    uint8_t mod_time_trial_leaderboard_car;
    uint32_t mod_time_trial_completed_laps[8];
    uint32_t mod_time_trial_completed_s1[8];
    uint32_t mod_time_trial_completed_s2[8];
    uint32_t mod_time_trial_completed_s3[8];
    TopGearTimeTrialRun mod_time_trial_completed_run;
    TopGearTimeTrialLeaderboardProvider mod_time_trial_leaderboard_provider;
    void *mod_time_trial_leaderboard_user;
    uint8_t mod_menu_post_race_context;
    uint8_t mod_music_player_track,mod_music_player_state;
    uint8_t mod_career_panel;
    uint8_t mod_continues_remaining,mod_continue_screen,mod_continue_selection,mod_continue_released;
    uint8_t mod_continue_course,mod_continue_rally,mod_attempt_live,mod_password_released;
    uint8_t mod_attempt_points[20];
    uint16_t mod_continue_previous;
    uint32_t mod_password_seed;
    uint8_t mod_career_qualification_pending,mod_career_password_award;
    uint8_t mod_career_password_open,mod_career_password_country,mod_career_password_error;
    uint8_t mod_career_start_course,mod_career_start_skill;
    uint8_t mod_career_order[20],mod_career_laps[20];
    uint16_t mod_career_progress[20],mod_career_last_tick;
    uint32_t mod_career_crossings[20][91],mod_career_last_lap[20];
    uint8_t mod_time_trial_font_valid;
    uint8_t mod_time_trial_font_reserved[3];
    uint8_t mod_time_trial_font_tiles[48][32];
    uint16_t mod_time_trial_font_palette[16];
};
...
int tg_audio_backend_mod_write_aram(struct TopGearRecomp*,uint32_t,const void*,size_t);
int tg_audio_backend_mod_write_dsp_register(struct TopGearRecomp*,uint8_t,uint8_t);
int tg_mod_insert_audio_process(struct TopGearRecomp*,uint64_t);
int tg_mod_menu_remodel_main_menu_enter(struct TopGearRecomp*);
void tg_mod_menu_remodel_draw_if_dirty(struct TopGearRecomp*);
int tg_mod_menu_remodel_handle_input(struct TopGearRecomp*,uint16_t,uint16_t*);
void tg_mod_rally_note_points_award(struct TopGearRecomp*);
unsigned tg_mod_rally_cutoff(unsigned index);
int tg_mod_rally_qualification_passes(struct TopGearRecomp*);
int tg_mod_rally_hide_failure_password(const struct TopGearRecomp*);
void tg_mod_rally_qualification_failed(struct TopGearRecomp*);
int tg_mod_rally_post_results(struct TopGearRecomp*);
uint16_t tg_mod_menu_remodel_selector_coord(struct TopGearRecomp*,uint16_t,uint16_t);
uint16_t tg_mod_menu_remodel_selector_width(struct TopGearRecomp*,uint16_t,uint16_t);
void tg_mod_rally_generate_schedule(struct TopGearRecomp*);
int tg_mod_time_trial_start(struct TopGearRecomp*);
int tg_mod_time_trial_post_results(struct TopGearRecomp*);
int tg_mod_time_trial_skip_career_points(const struct TopGearRecomp*);
void tg_mod_time_trial_note_lap(struct TopGearRecomp*,uint16_t);
void tg_mod_time_trial_capture_required_laps(struct TopGearRecomp*,uint16_t);
const char *tg_mod_music_player_label(const TopGearRecomp *i);
void tg_mod_music_player_action(TopGearRecomp *i,int direction);
void tg_mod_attempt_begin(TopGearRecomp *);
void tg_mod_attempt_race(TopGearRecomp *);
void tg_mod_attempt_fail(TopGearRecomp *);
int tg_mod_attempt_pre_results(TopGearRecomp *);
int tg_mod_continue_poll(TopGearRecomp *);
unsigned tg_mod_continue_complete(TopGearRecomp *);
void tg_mod_password_standings(TopGearRecomp *);
...
unsigned tg_mod_career_cutoff(unsigned course);
unsigned tg_mod_career_retry_course(const TopGearRecomp *i);
unsigned tg_mod_career_password_count(const TopGearRecomp *i);
unsigned tg_mod_career_password_country(const TopGearRecomp *i,unsigned selection);
const char *tg_mod_career_skill_name(const TopGearRecomp *i);
void tg_mod_career_validate_start(TopGearRecomp *i);
int tg_mod_career_country_passes(struct TopGearRecomp*);
...
void tg_mod_time_trial_frame(struct TopGearRecomp*);
void tg_mod_time_trial_overlay_scanline(struct TopGearRecomp*,unsigned);
int tg_mod_time_trial_commit_completed_run(struct TopGearRecomp*);
void tg_mod_audio_gameplay_start(struct TopGearRecomp*);
uint16_t tg_mod_music_input(struct TopGearRecomp*,uint16_t);
int tg_mod_music_race_prepare(struct TopGearRecomp*);
void tg_mod_music_menu_enter(struct TopGearRecomp*);
void tg_mod_music_rally_playlist(struct TopGearRecomp*);
unsigned tg_mod_music_pick_song(struct TopGearRecomp*);
int tg_mod_insert_audio_main_menu_enter(struct TopGearRecomp*);
void tg_mod_insert_audio_start_race(struct TopGearRecomp*);
void tg_mod_insert_audio_accept(struct TopGearRecomp*);
void tg_mod_insert_audio_navigation(TopGearRecomp *instance);
void tg_mod_insert_audio_menu_selection(struct TopGearRecomp*,uint16_t);
