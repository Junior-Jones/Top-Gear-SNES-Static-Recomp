...
int g_acquired=0;
constexpr size_t kDspSnapshotCapacity=4096u;
...
void save_dsp_bytes(unsigned char **io,void *state,size_t size){std::memcpy(*io,state,size);*io+=size;}
void load_dsp_bytes(unsigned char **io,void *state,size_t size){std::memcpy(state,*io,size);*io+=size;}
...
void drain_samples(){
  int count=SC_STATIC_SNES::dsp.spc_dsp.sample_count();
  if(count<0) count=0;
  if((size_t)count>kScratchSamples) count=(int)kScratchSamples;
  count&=~1;
  for(int i=0;i<count;i+=2){
    if(g_sink) g_sink(g_sink_context,g_samples[i],g_samples[i+1]);
    ++g_pcm_frames;
  }
}
void record_rendezvous(uint64_t master,uint64_t target_cycles,uint64_t master_delta){
  ...
  h=mix8(h,(uint8_t)SC_STATIC_SNES::dsp.spc_dsp.phase_value());
...
int fail_message(char *error,size_t cap){
  ...
  const char *reason=s.sc_aot_fail_reason_value==1u?"unknown exact S-SMP PC":
    ...
    s.sc_aot_fail_reason_value==4u?"write to statically compiled S-SMP code":"static S-SMP AOT failure";
...
int sync_impl(uint64_t master,char *error,size_t cap){
  ...
  while(delta){
    ...
    SC_STATIC_SNES::dsp.spc_dsp.set_output(g_samples,(int)kScratchSamples);
...
extern "C" int sc_static_apu_status(SCStaticApuStatus *s){
  ...
  s->pcm_frames=g_pcm_frames;s->smp_pc=SC_STATIC_SNES::smp.regs.pc;
  ...
  s->dsp_phase=(uint8_t)SC_STATIC_SNES::dsp.spc_dsp.phase_value();s->timer_enable_mask=timer_enable_mask();
...
extern "C" int sc_static_apu_read_dsp_register(uint8_t address,uint8_t *value){
  ...
  *value=(uint8_t)SC_STATIC_SNES::dsp.spc_dsp.read(address);
  return 1;
}

extern "C" size_t sc_static_apu_snapshot_size(void){return sizeof(ApuSnapshotHeader)+sizeof(SC_STATIC_SNES::SMP)+65536u+kDspSnapshotCapacity;}
extern "C" int sc_static_apu_snapshot_save(void *data,size_t capacity){
  ApuSnapshotHeader h{};unsigned char *out=(unsigned char*)data,*dsp_start,*dsp_out;
  if(!g_acquired||!data||capacity<sc_static_apu_snapshot_size())return 0;
  std::memcpy(h.magic,"TGAPU002",8u);h.version=2u;h.smp_size=(uint32_t)sizeof(SC_STATIC_SNES::SMP);h.aram_size=65536u;h.dsp_capacity=(uint32_t)kDspSnapshotCapacity;
  ...
  std::memcpy(out,SC_STATIC_SNES::smp.apuram,65536u);out+=65536u;dsp_start=out;std::memset(dsp_start,0,kDspSnapshotCapacity);dsp_out=dsp_start;
  SC_STATIC_SNES::dsp.spc_dsp.copy_state(&dsp_out,save_dsp_bytes);return (size_t)(dsp_out-dsp_start)<=kDspSnapshotCapacity;
}
extern "C" int sc_static_apu_snapshot_load(const void *data,size_t size,char *error,size_t error_capacity){
  ApuSnapshotHeader h;const unsigned char *in=(const unsigned char*)data,*dsp_start;unsigned char *dsp_in;uint8 *apuram;
  ...
  if(std::memcmp(h.magic,"TGAPU002",8u)!=0||h.version!=2u||h.smp_size!=sizeof(SC_STATIC_SNES::SMP)||h.aram_size!=65536u||h.dsp_capacity!=kDspSnapshotCapacity){copy_error(error,error_capacity,"Static audio snapshot is from another build.");return 0;}
  apuram=SC_STATIC_SNES::smp.apuram;std::memcpy(&SC_STATIC_SNES::smp,in,sizeof(SC_STATIC_SNES::smp));SC_STATIC_SNES::smp.apuram=apuram;in+=sizeof(SC_STATIC_SNES::smp);
  std::memcpy(apuram,in,65536u);in+=65536u;std::memcpy(SC_STATIC_SNES::cpu.registers,h.cpu_registers,4u);SC_STATIC_SNES::dsp.frequency=h.dsp_frequency;SC_STATIC_SNES::dsp.clock=h.dsp_clock;
  dsp_start=in;dsp_in=(unsigned char*)dsp_start;SC_STATIC_SNES::dsp.spc_dsp.copy_state(&dsp_in,load_dsp_bytes);
  if((size_t)(dsp_in-dsp_start)>kDspSnapshotCapacity){copy_error(error,error_capacity,"Static audio DSP snapshot is invalid.");return 0;}
