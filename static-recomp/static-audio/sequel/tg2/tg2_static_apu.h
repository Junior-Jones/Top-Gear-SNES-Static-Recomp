...
int tg2_static_apu_acquire(char *error,size_t error_capacity);
void tg2_static_apu_release(void);
void tg2_static_apu_reset(void);
void tg2_static_apu_set_sink(SCStaticAudioSink sink,void *context);
void tg2_static_apu_set_trace_callbacks(const SCStaticTraceCallbacks *callbacks);
void tg2_static_apu_trace_instruction_event(uint16_t pc,uint8_t opcode);
int tg2_static_apu_trace_aram_write_event(uint16_t address,uint8_t value);
void tg2_static_apu_trace_dsp_write_event(uint8_t address,uint8_t value);
void tg2_static_apu_trace_port_event(uint8_t direction,uint8_t port,uint8_t value);
void tg2_static_sdsp_primitive_step(uint8_t phase);
int tg2_static_sdsp_brr_step(uint16_t address,const uint8_t *aram);
void tg2_static_sdsp_report_failure(uint32_t reason,uint8_t phase);
int tg2_static_apu_sync_to_master(uint64_t master_clock,char *error,size_t error_capacity);
int tg2_static_apu_cpu_write_port(uint64_t master_clock,unsigned port,uint8_t value,char *error,size_t error_capacity);
uint8_t tg2_static_apu_cpu_read_port(uint64_t master_clock,unsigned port,int *ok,char *error,size_t error_capacity);
int tg2_static_apu_status(SCStaticApuStatus *status);
int tg2_static_apu_read_aram(uint32_t offset,void *output,size_t bytes);
...
int tg2_static_apu_mod_write_aram(uint32_t offset,const void *input,size_t bytes);
int tg2_static_apu_mod_write_dsp_register(uint8_t address,uint8_t value);
...
int tg2_static_apu_mod_set_menu_voice7_only(int enabled);
int tg2_static_apu_mod_set_menu_music_gain(uint8_t percent);
int tg2_static_apu_filter_game_dsp_write(uint8_t address,uint8_t *value);
int tg2_static_apu_read_dsp_register(uint8_t address,uint8_t *value);
/* Top Gear 2 standalone laboratory helpers. These are not exposed by the Top Gear production core. */
int tg2_static_apu_lab_seed_tg2(const void *aram_image,size_t bytes,uint8_t dsp_phase);
int tg2_static_apu_lab_force_pc(uint16_t pc);

size_t tg2_static_apu_pcm_available(void);
size_t tg2_static_apu_pcm_read(int16_t *interleaved_stereo,uint8_t *frame_known,
                              size_t capacity_frames);
uint64_t tg2_static_apu_pcm_overflow_count(void);
size_t tg2_static_apu_snapshot_size(void);
int tg2_static_apu_snapshot_save(void *data,size_t capacity);
int tg2_static_apu_snapshot_load(const void *data,size_t size,
