...
#include <cstdio>
#include <cstring>
namespace TG3_STATIC_SNES { CPU cpu; }
namespace {
...
int g_acquired=0;
bool g_menu_voice7_only=false;
uint8_t g_menu_music_gain_percent=100u;
struct ApuSnapshotHeader {
  ...
  uint8_t menu_filter_active,menu_music_gain_percent,reserved_mod_audio[2];
};
...
uint8_t timer_enable_mask(){return (TG3_STATIC_SNES::smp.timer0.enable?1u:0u)|(TG3_STATIC_SNES::smp.timer1.enable?2u:0u)|(TG3_STATIC_SNES::smp.timer2.enable?4u:0u);}
...
void drain_samples(){
  ...
  size_t count;
  do {
    count=topgear_dsp_pcm_read_with_knownness(&TG3_STATIC_SNES::dsp.core,samples,known,512u);
    for(size_t i=0;i<count;++i){
      if(g_sink) g_sink(g_sink_context,samples[i*2u],samples[i*2u+1u]);
      ++g_pcm_frames;
    }
  } while(count==512u);
  if(TG3_STATIC_SNES::dsp.core.last_stop!=TOPGEAR_DSP_STOP_NONE){
    tg3_static_sdsp_report_failure((uint32_t)TG3_STATIC_SNES::dsp.core.last_stop,
                                  TG3_STATIC_SNES::dsp.core.phase);
  }
}
void record_rendezvous(uint64_t master,uint64_t target_cycles,uint64_t master_delta){
  ...
  h=mix32(h,(uint32_t)TG3_STATIC_SNES::smp.clock);
  h=mix8(h,(uint8_t)TG3_STATIC_SNES::dsp.core.phase);
  h=mix8(h,timer_enable_mask());
  mix_timer(h,TG3_STATIC_SNES::smp.timer0);
  mix_timer(h,TG3_STATIC_SNES::smp.timer1);
  mix_timer(h,TG3_STATIC_SNES::smp.timer2);
  g_rendezvous_hash=h;
}
...
int fail_message(char *error,size_t cap){
  const auto &s=TG3_STATIC_SNES::smp;
  char text[224];
...
int sync_impl(uint64_t master,char *error,size_t cap){
  ...
  if(TG3_STATIC_SNES::smp.sc_aot_failed_value)return fail_message(error,cap);
  ...
  while(delta){
    ...
    TG3_STATIC_SNES::smp.clock-=(int32)chunk;
    TG3_STATIC_SNES::smp.enter();
    if(TG3_STATIC_SNES::smp.sc_aot_failed_value)return fail_message(error,cap);
    TG3_STATIC_SNES::dsp.synchronize();drain_samples();g_cycles+=chunk;delta-=chunk;
  }
  g_master=master;record_rendezvous(master,target,master_delta);copy_error(error,cap,"");return 1;
}
}
extern "C" int tg3_static_apu_acquire(char *error,size_t cap){
  if(g_acquired){copy_error(error,cap,"Only one Full Static S-SMP AOT instance is supported.");return 0;}
  g_acquired=1;tg3_static_apu_reset();copy_error(error,cap,"");return 1;
}
extern "C" void tg3_static_apu_release(void){g_acquired=0;g_sink=nullptr;g_sink_context=nullptr;std::memset(&g_trace,0,sizeof(g_trace));}
extern "C" void tg3_static_apu_reset(void){
  TG3_STATIC_SNES::cpu.reset();TG3_STATIC_SNES::smp.power();TG3_STATIC_SNES::dsp.power();
  ...
  g_menu_voice7_only=false;g_menu_music_gain_percent=100u;
}
extern "C" void tg3_static_apu_set_sink(SCStaticAudioSink sink,void *context){g_sink=sink;g_sink_context=context;}
extern "C" void tg3_static_apu_set_trace_callbacks(const SCStaticTraceCallbacks *callbacks){if(callbacks)g_trace=*callbacks;else std::memset(&g_trace,0,sizeof(g_trace));}
extern "C" void tg3_static_apu_trace_instruction_event(uint16_t pc,uint8_t opcode){
  g_current_instruction_pc=pc;if(pc==0x0800u||pc==0x0810u||pc==0x0840u)g_program_started=true;
  if(g_trace.instruction)g_trace.instruction(g_trace.context,pc,opcode);
}
extern "C" int tg3_static_apu_trace_aram_write_event(uint16_t address,uint8_t value){
...
extern "C" void tg3_static_apu_trace_dsp_write_event(uint8_t address,uint8_t value){if(g_trace.dsp_write)g_trace.dsp_write(g_trace.context,g_current_instruction_pc,address,value);}
extern "C" void tg3_static_apu_trace_port_event(uint8_t direction,uint8_t port,uint8_t value){if(g_trace.port)g_trace.port(g_trace.context,g_current_instruction_pc,direction,port,value);}
extern "C" void tg3_static_sdsp_primitive_step(uint8_t phase){
...
extern "C" int tg3_static_sdsp_brr_step(uint16_t address,const uint8_t *aram){
...
extern "C" void tg3_static_sdsp_report_failure(uint32_t reason,uint8_t phase){
  if(!g_sdsp_static_failed){g_sdsp_static_failed=1u;g_sdsp_fail_reason=(uint8_t)(reason&0xffu);g_sdsp_fail_phase=phase;}
}
extern "C" int tg3_static_apu_sync_to_master(uint64_t m,char *e,size_t c){return sync_impl(m,e,c);}
extern "C" int tg3_static_apu_cpu_write_port(uint64_t m,unsigned p,uint8_t v,char *e,size_t c){
  if(p>=4u){copy_error(e,c,"Invalid static APUIO port.");return 0;}if(!sync_impl(m,e,c))return 0;
  TG3_STATIC_SNES::cpu.port_write((uint8)p,v);record_cpu_port(1u,m,p,v);return 1;
}
extern "C" uint8_t tg3_static_apu_cpu_read_port(uint64_t m,unsigned p,int *ok,char *e,size_t c){
  if(ok) *ok=0;
  if(p>=4u){copy_error(e,c,"Invalid static APUIO port.");return 0;}
  if(!sync_impl(m,e,c)) return 0;
  const uint8_t value=(uint8_t)TG3_STATIC_SNES::smp.port_read(p);
  record_cpu_port(0u,m,p,value);
  if(ok) *ok=1;
  return value;
}
extern "C" int tg3_static_apu_status(SCStaticApuStatus *s){
  if(!s||!g_acquired) return 0;
  std::memset(s,0,sizeof(*s));
  s->synchronized_master_clock=g_master;s->smp_cycles=g_cycles;
  s->smp_cycle_overshoot=TG3_STATIC_SNES::smp.clock;s->smp_executed_cycles=g_cycles+(uint64_t)(TG3_STATIC_SNES::smp.clock<0?0:TG3_STATIC_SNES::smp.clock);
  s->smp_instructions=TG3_STATIC_SNES::smp.instruction_count;s->aot_validated_instructions=TG3_STATIC_SNES::smp.sc_aot_instructions;
  s->pcm_frames=TG3_STATIC_SNES::dsp.core.pcm_frames_produced;
  s->pcm_known_frames=TG3_STATIC_SNES::dsp.core.pcm_known_frames_produced;
  s->pcm_unknown_frames=TG3_STATIC_SNES::dsp.core.pcm_unknown_frames_produced;
  s->pcm_hash=TG3_STATIC_SNES::dsp.core.pcm_fnv1a64;
  s->pcm_overflows=TG3_STATIC_SNES::dsp.core.pcm_overflows;
  s->pcm_available=TG3_STATIC_SNES::dsp.core.pcm_count;
  s->smp_pc=TG3_STATIC_SNES::smp.regs.pc;
  s->smp_a=TG3_STATIC_SNES::smp.regs.B.a;s->smp_x=TG3_STATIC_SNES::smp.regs.x;s->smp_y=TG3_STATIC_SNES::smp.regs.B.y;s->smp_sp=TG3_STATIC_SNES::smp.regs.sp;s->smp_psw=(uint8_t)(unsigned)TG3_STATIC_SNES::smp.regs.p;
  s->aot_fail_pc=TG3_STATIC_SNES::smp.sc_aot_fail_pc_value;
  s->expected_opcode=TG3_STATIC_SNES::smp.sc_aot_expected_opcode;s->actual_opcode=TG3_STATIC_SNES::smp.sc_aot_actual_opcode;
  s->aot_failed=(uint8_t)(TG3_STATIC_SNES::smp.sc_aot_failed_value!=0u);s->aot_fail_reason=(uint8_t)TG3_STATIC_SNES::smp.sc_aot_fail_reason_value;
  s->current_epoch=(uint8_t)(TG3_STATIC_SNES::smp.status.iplrom_enable?0u:1u);s->code_write_barriers=g_code_write_barriers;
  ...
  s->dsp_phase=(uint8_t)TG3_STATIC_SNES::dsp.core.phase;s->timer_enable_mask=timer_enable_mask();
  store_timer(s->timer_target,s->timer_stage1,s->timer_stage2,s->timer_stage3,0u,TG3_STATIC_SNES::smp.timer0);
  store_timer(s->timer_target,s->timer_stage1,s->timer_stage2,s->timer_stage3,1u,TG3_STATIC_SNES::smp.timer1);
  store_timer(s->timer_target,s->timer_stage1,s->timer_stage2,s->timer_stage3,2u,TG3_STATIC_SNES::smp.timer2);
...
extern "C" int tg3_static_apu_read_aram(uint32_t offset,void *output,size_t bytes){
  if(!g_acquired||!output||offset>65536u||bytes>65536u-offset)return 0;
  std::memcpy(output,TG3_STATIC_SNES::smp.apuram+offset,bytes);
  return 1;
}

extern "C" int tg3_static_apu_mod_write_aram(uint32_t offset,const void *input,size_t bytes){
  ...
  for(size_t n=0;n<bytes;++n){
    const uint16_t address=(uint16_t)(offset+n);
    if(is_code_byte(address))return 0;
  }
  for(size_t n=0;n<bytes;++n){
    const uint16_t address=(uint16_t)(offset+n);
    TG3_STATIC_SNES::smp.apuram[address]=source[n];
    TG3_STATIC_SNES::smp.mark_aram_known(address);
  }
  return 1;
}

extern "C" int tg3_static_apu_mod_write_dsp_register(uint8_t address,uint8_t value){
  if(!g_acquired||address>=128u)return 0;
  TG3_STATIC_SNES::dsp.synchronize();
  TG3_STATIC_SNES::dsp.write(address,value);
  return g_sdsp_static_failed?0:1;
}

extern "C" int tg3_static_apu_filter_game_dsp_write(uint8_t address,uint8_t *value){return value&&address<128u;}
extern "C" int tg3_static_apu_mod_set_menu_music_gain(uint8_t percent){if(percent>100)return 0;topgear_dsp_music_gain(&TG3_STATIC_SNES::dsp.core,percent);return 1;}
extern "C" int tg3_static_apu_mod_set_menu_voice7_only(int enabled){return !enabled;}

extern "C" int tg3_static_apu_read_dsp_register(uint8_t address,uint8_t *value){
  ...
  TG3_STATIC_SNES::dsp.synchronize();
  *value=TG3_STATIC_SNES::dsp.core.regs[address&0x7fu];
  return 1;
}

extern "C" size_t tg3_static_apu_pcm_available(void){return g_acquired?topgear_dsp_pcm_available(&TG3_STATIC_SNES::dsp.core):0u;}
extern "C" size_t tg3_static_apu_pcm_read(int16_t *out,uint8_t *known,size_t capacity){
  if(!g_acquired||!out)return 0u;
  return topgear_dsp_pcm_read_with_knownness(&TG3_STATIC_SNES::dsp.core,out,known,capacity);
}
extern "C" uint64_t tg3_static_apu_pcm_overflow_count(void){return g_acquired?TG3_STATIC_SNES::dsp.core.pcm_overflows:0u;}

extern "C" size_t tg3_static_apu_snapshot_size(void){return sizeof(ApuSnapshotHeader)+sizeof(TG3_STATIC_SNES::SMP)+65536u+8192u+sizeof(topgear_dsp);}
extern "C" int tg3_static_apu_snapshot_save(void *data,size_t capacity){
  ApuSnapshotHeader h{};unsigned char *out=(unsigned char*)data;topgear_dsp dsp_state;
  if(!g_acquired||!data||capacity<tg3_static_apu_snapshot_size())return 0;
  std::memcpy(h.magic,"TGAPU004",8u);h.version=4u;h.smp_size=(uint32_t)sizeof(TG3_STATIC_SNES::SMP);h.aram_size=65536u;h.dsp_capacity=(uint32_t)sizeof(topgear_dsp);
  std::memcpy(h.cpu_registers,TG3_STATIC_SNES::cpu.registers,4u);h.dsp_frequency=TG3_STATIC_SNES::dsp.frequency;h.dsp_clock=TG3_STATIC_SNES::dsp.clock;
  ...
  h.menu_filter_active=g_menu_voice7_only?1u:0u;h.menu_music_gain_percent=g_menu_music_gain_percent;h.reserved_mod_audio[0]=h.reserved_mod_audio[1]=0u;
  std::memcpy(out,&h,sizeof(h));out+=sizeof(h);std::memcpy(out,&TG3_STATIC_SNES::smp,sizeof(TG3_STATIC_SNES::smp));out+=sizeof(TG3_STATIC_SNES::smp);
  std::memcpy(out,TG3_STATIC_SNES::smp.apuram,65536u);out+=65536u;
  std::memcpy(out,TG3_STATIC_SNES::smp.aram_known,8192u);out+=8192u;
  dsp_state=TG3_STATIC_SNES::dsp.core;dsp_state.aram=nullptr;dsp_state.aram_known=nullptr;
  std::memcpy(out,&dsp_state,sizeof(dsp_state));return 1;
}
extern "C" int tg3_static_apu_snapshot_load(const void *data,size_t size,char *error,size_t error_capacity){
  ApuSnapshotHeader h;const unsigned char *in=(const unsigned char*)data;uint8 *apuram,*aram_known;
  if(!g_acquired||!data||size!=tg3_static_apu_snapshot_size()){copy_error(error,error_capacity,"Static audio snapshot size is invalid.");return 0;}
  std::memcpy(&h,in,sizeof(h));in+=sizeof(h);
  if(std::memcmp(h.magic,"TGAPU004",8u)!=0||h.version!=4u||h.smp_size!=sizeof(TG3_STATIC_SNES::SMP)||h.aram_size!=65536u||h.dsp_capacity!=sizeof(topgear_dsp)){copy_error(error,error_capacity,"Static audio snapshot is from another build.");return 0;}
  apuram=TG3_STATIC_SNES::smp.apuram;aram_known=TG3_STATIC_SNES::smp.aram_known;
  std::memcpy(&TG3_STATIC_SNES::smp,in,sizeof(TG3_STATIC_SNES::smp));TG3_STATIC_SNES::smp.apuram=apuram;TG3_STATIC_SNES::smp.aram_known=aram_known;in+=sizeof(TG3_STATIC_SNES::smp);
  std::memcpy(apuram,in,65536u);in+=65536u;std::memcpy(aram_known,in,8192u);in+=8192u;
  std::memcpy(TG3_STATIC_SNES::cpu.registers,h.cpu_registers,4u);TG3_STATIC_SNES::dsp.frequency=h.dsp_frequency;TG3_STATIC_SNES::dsp.clock=h.dsp_clock;
  std::memcpy(&TG3_STATIC_SNES::dsp.core,in,sizeof(TG3_STATIC_SNES::dsp.core));TG3_STATIC_SNES::dsp.core.aram=apuram;TG3_STATIC_SNES::dsp.core.aram_known=aram_known;
  ...
  g_menu_voice7_only=h.menu_filter_active!=0u;g_menu_music_gain_percent=h.menu_music_gain_percent<=100u?h.menu_music_gain_percent:100u;
  /* The raw DSP snapshot contains the accumulator/gain state.  Reapply only
     the public mode fields defensively so wrapper and mixer cannot diverge. */
  /* Complete project DSP gain is restored directly from its snapshot. */
  copy_error(error,error_capacity,"");return 1;
}

extern "C" int tg3_static_apu_lab_seed_tg2(const void *aram_image,size_t bytes,uint8_t dsp_phase){
  if(!g_acquired||!aram_image||bytes!=65536u||dsp_phase>=32u)return 0;
  tg3_static_apu_reset();
  std::memcpy(TG3_STATIC_SNES::smp.apuram,aram_image,65536u);
  std::memset(TG3_STATIC_SNES::smp.aram_known,0xff,8192u);
  TG3_STATIC_SNES::smp.regs.pc=0x0810u;
  TG3_STATIC_SNES::smp.regs.sp=0xefu;
  TG3_STATIC_SNES::smp.regs.B.a=0u;TG3_STATIC_SNES::smp.regs.x=0u;TG3_STATIC_SNES::smp.regs.B.y=0u;TG3_STATIC_SNES::smp.regs.p=0x02u;
  TG3_STATIC_SNES::smp.status.iplrom_enable=false;
  TG3_STATIC_SNES::smp.clock=0;
  TG3_STATIC_SNES::dsp.core.phase=dsp_phase;
  g_program_started=true;g_current_instruction_pc=0x0810u;
  return 1;
}

extern "C" int tg3_static_apu_lab_seed_tg3(const void *aram_image,size_t bytes,uint8_t dsp_phase){
  if(!g_acquired||!aram_image||bytes!=65536u||dsp_phase>=32u)return 0;
  tg3_static_apu_reset();
  std::memcpy(TG3_STATIC_SNES::smp.apuram,aram_image,65536u);
  std::memset(TG3_STATIC_SNES::smp.aram_known,0xff,8192u);
  TG3_STATIC_SNES::smp.regs.pc=0x0840u;
  TG3_STATIC_SNES::smp.regs.sp=0xefu;
  TG3_STATIC_SNES::smp.regs.B.a=0u;TG3_STATIC_SNES::smp.regs.x=0u;TG3_STATIC_SNES::smp.regs.B.y=0u;TG3_STATIC_SNES::smp.regs.p=0x02u;
  TG3_STATIC_SNES::smp.status.iplrom_enable=false;
  TG3_STATIC_SNES::smp.clock=0;
  TG3_STATIC_SNES::dsp.core.phase=dsp_phase;
  g_program_started=true;g_current_instruction_pc=0x0840u;
  return 1;
}
extern "C" int tg3_static_apu_lab_force_pc(uint16_t pc){if(!g_acquired)return 0;TG3_STATIC_SNES::smp.regs.pc=pc;return 1;}
