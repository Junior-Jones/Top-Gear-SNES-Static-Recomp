...
struct Capture { bool collecting=false; std::vector<int16_t> pcm; uint8_t nonzero_volume_write_mask=0; uint8_t kon_touch_mask=0; uint8_t koff_touch_mask=0; uint8_t eon_mask=0; uint8_t pmon_mask=0; uint8_t non_mask=0; };
...
static bool load_transfers(const uint8_t *b,size_t size,std::vector<Transfer>&out){
 if(size<12||std::memcmp(b,"TG3XFER1",8))return false;
...
extern "C" int tg3_music_select(unsigned track){
 if(track>=7u)return 0;
 char error[256];
 if(!acquired){if(!tg3_static_apu_acquire(error,sizeof(error)))return 0;acquired=true;}
 tg3_static_apu_set_sink(sink,&capture);tg3_static_apu_set_trace_callbacks(nullptr);
 capture.collecting=false;capture.pcm.clear();readIndex=0;
 if(cache[track].bytes.empty()){
  if(!tg3_static_apu_lab_seed_tg3(tg3_driver_base,sizeof(tg3_driver_base),20))return 0;
  clockMaster=0;std::vector<Transfer> xfers;
unsigned package=track;
switch(package){
case 0:if(!load_transfers(tg3_package_0_transfer,sizeof(tg3_package_0_transfer),xfers))return 0;break;
case 1:if(!load_transfers(tg3_package_1_transfer,sizeof(tg3_package_1_transfer),xfers))return 0;break;
case 2:if(!load_transfers(tg3_package_2_transfer,sizeof(tg3_package_2_transfer),xfers))return 0;break;
case 3:if(!load_transfers(tg3_package_3_transfer,sizeof(tg3_package_3_transfer),xfers))return 0;break;
case 4:if(!load_transfers(tg3_package_4_transfer,sizeof(tg3_package_4_transfer),xfers))return 0;break;
case 5:if(!load_transfers(tg3_package_5_transfer,sizeof(tg3_package_5_transfer),xfers))return 0;break;
case 6:if(!load_transfers(tg3_package_6_transfer,sizeof(tg3_package_6_transfer),xfers))return 0;break;
default:return 0;}
if(!wait_ready77(clockMaster,12000000))return 0;uint64_t preService=clockMaster+357366;if(!run_prepackage_until(clockMaster,96888258ull,preService)||!load_package(clockMaster,xfers)||!start_loaded_song(clockMaster))return 0;
serviceNext=clockMaster+357366;cache[track].bytes.resize(tg3_static_apu_snapshot_size());
 if(!tg3_static_apu_snapshot_save(cache[track].bytes.data(),cache[track].bytes.size()))return 0;
 cache[track].master=clockMaster;cache[track].next=serviceNext;
 }else{if(!tg3_static_apu_snapshot_load(cache[track].bytes.data(),cache[track].bytes.size(),error,sizeof(error)))return 0;clockMaster=cache[track].master;serviceNext=cache[track].next;}
 capture.collecting=true;return 1;
}
extern "C" int tg3_music_sample(int16_t *left,int16_t *right){
...
extern "C" void tg3_music_gain(unsigned percent){tg3_static_apu_mod_set_menu_music_gain((uint8_t)percent);}
extern "C" int tg3_music_prepare(void){for(unsigned t=0;t<7;t++)if(!tg3_music_select(t))return 0;return 1;}
extern "C" void tg3_music_release(void){if(acquired)tg3_static_apu_release();acquired=false;capture.collecting=false;capture.pcm.clear();readIndex=0;}

struct RuntimeSnapshot {uint64_t master,next;uint32_t count,index,active,reserved;int16_t pending[1024];};
extern "C" size_t tg3_music_snapshot_size(void){return sizeof(RuntimeSnapshot)+tg3_static_apu_snapshot_size();}
extern "C" int tg3_music_snapshot_save(void *data,size_t size){
 if(size!=tg3_music_snapshot_size()||capture.pcm.size()>1024)return 0;RuntimeSnapshot h{};h.master=clockMaster;h.next=serviceNext;h.count=(uint32_t)capture.pcm.size();h.index=(uint32_t)readIndex;h.active=capture.collecting?1:0;std::memcpy(h.pending,capture.pcm.data(),h.count*2);std::memcpy(data,&h,sizeof(h));return tg3_static_apu_snapshot_save((uint8_t*)data+sizeof(h),size-sizeof(h));}
extern "C" int tg3_music_snapshot_load(const void *data,size_t size,char *error,size_t capacity){
 if(size!=tg3_music_snapshot_size())return 0;RuntimeSnapshot h;std::memcpy(&h,data,sizeof(h));if(h.count>1024||h.index>h.count||(h.count&1)||(h.index&1)||h.active>1||h.next<h.master)return 0;
 if(!tg3_static_apu_snapshot_load((const uint8_t*)data+sizeof(h),size-sizeof(h),error,capacity))return 0;clockMaster=h.master;serviceNext=h.next;capture.pcm.assign(h.pending,h.pending+h.count);readIndex=h.index;capture.collecting=h.active!=0;tg3_static_apu_set_sink(sink,&capture);return 1;}
