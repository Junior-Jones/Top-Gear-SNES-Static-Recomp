...
void SMP::op_write(uint16 addr, uint8 data) {
  tick();
  if(!tg3_static_apu_trace_aram_write_event(addr,data)) {
#ifdef SC_SMP_AOT
...
void SMP::op_writestack(uint8 data)
{
  tick();
  const uint16 addr=(uint16)(0x0100 | regs.sp);
  if(tg3_static_apu_trace_aram_write_event(addr,data)){apuram[addr]=data;mark_aram_known(addr);}
  else {
#ifdef SC_SMP_AOT
    sc_aot_fail(4u,regs.pc,0xffu,data);
#endif
...
void SMP::op_step() {
  ...
  if(opcode_cycle == 0)
  {
    ...
    tg3_static_apu_trace_instruction_event(opcode_pc, static_cast<uint8>(opcode_number));
