...
unsigned SMP::mmio_read(unsigned addr) {
  ...
  case 0xf7: {
    ...
    tg3_static_apu_trace_port_event(0u,(uint8)(addr-0xf4u),value);
    return value;
  }
...
void SMP::mmio_write(unsigned addr, unsigned data) {
  ...
  case 0xf3:
    ...
    tg3_static_apu_trace_dsp_write_event((uint8)status.dsp_addr,(uint8)data);
    {
      uint8 effective=(uint8)data;
      if(tg3_static_apu_filter_game_dsp_write((uint8)status.dsp_addr,&effective))
        dsp.write(status.dsp_addr,effective);
    }
    break;
  ...
  case 0xf6:
  case 0xf7:
    tg3_static_apu_trace_port_event(1u,(uint8)(addr-0xf4u),(uint8)data);
